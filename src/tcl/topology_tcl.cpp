...
int tclcommand_analyze_set_print_all(Tcl_Interp *interp)
{
  ...
  for (m = 0; m < topology.size(); m++) {
    ...
    for (i = 0; i < topology[m].part.size(); i++) {
      sprintf(buffer, "%d ", topology[m].part[i]);
      Tcl_AppendResult(interp, buffer, (char *)NULL);      
    }
...
int tclcommand_analyze_parse_generic_structure(Tcl_Interp *interp, int argc, char **argv)
{
  ...
  for (arg = 0; arg < argc; arg++) {
    ...
    topology[arg].part.resize(il.n - 1);

    std::copy(il.e + 1, il.e + il.n, topology[arg].part.begin());
  }
  realloc_intlist(&il, 0);
...
int tclcommand_analyze_set_parse_topo_part_sync(Tcl_Interp *interp) {
  ...
  for ( i = 0 ; i < topology.size() ; i ++ ) {
    ntopoparts += topology[i].part.size();
  }
